...
There is a very close relationship between these triples and first order logic. The relations can be treated as *predicates* whilst the nodes can be treated as variables. This allows us to specify constraints on the graph. For example, if we wanted to specify that every animal must belong to a taxonomic class (eg mammal, reptile) we could write:

$(\forall x\in\{animals\})(\exists y\in\{class\})[is_a(x,y)]$

ie, for every animal, there is a class for which the positive relation `is_a` is true (ie is in the graph). We can in fact specify the whole of the ontology using this language, including relations that should not exist.

The triples representation has the distinct advantage of being very easy to parse, and can be translated almost directly into a computer program. However, it has the disadvantage of not containing any explanations. For this, mark-up languages are often used instead.
