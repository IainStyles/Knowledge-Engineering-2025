...
Let's run through a few simple examples for illustarive purposes. It is **very important** to note that proofs in first order logic can sometimes be very subtle and proof steps that might seem very natural to us would not typically be accepted by the standards of professional logicians. We will highlight a couple of examples of this. Our proofs will not, in some cases, adhere to the very highest standard of proof that would be accepted by a logician. 

One particular point to highlight in advance of these examples is that apply the rules of proof such as $^$-elimination under a quantifier is considered to be a rather dangerous thing to do. For the simple examples we will look at, this is unlikely to matter but we will typically proceed by eliminating the quantifiers, applying our rule, and then re-introducing the quantifiers. Let's do a simple example:

* $\forall x [P(x)\land Q(x)] : \forall x [P(x)]$

At first glance this appears to be proven straightforwardly by and-elimination. However, this would not usually be accepted as a vlaid proof strategy by logicians, who would prefer to proceed in the following way:
